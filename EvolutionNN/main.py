...
# initialize the one hot encoder and standard scaler
...
# apply the standard scaler to the input data and the one hot encoder to the output data
...
def calculate_cost_function(network_output, expected_output):
    ...
    actual_expected_zipped = np.array(list(zip(network_output, expected_output)))
    for pair in actual_expected_zipped:
        actual = pair[0][0]
        expected = pair[1][0]
        difference = actual - expected
        squared_difference = difference * difference
        cost_function_value += squared_difference
    return cost_function_value / len(actual_expected_zipped)
...
def get_accuracy_score(predicted_probability, actual_probability, samples_per_batch):
    if (predicted_probability.shape[0] is not actual_probability.shape[0]) or (predicted_probability.shape[1] is not actual_probability.shape[1]):
        ...
        correct_counter = 0
        wrong_counter = 0
        for distribution_index in range(samples_per_batch):
            model_choice = np.argmax(predicted_probability[distribution_index])
            correct_choice = np.argmax(actual_probability[distribution_index])
...
def train_test_split(input_data, expected_output_data, percent_train):
...
class layer_dense():
    ...
    def feedforward(self, input_activations, activation_func):
        ...
        if activation_func is 'relu':
            ...
            for activation_index in range(len(outputs)):
                activation_matrix = outputs[activation_index]
                exponent_activation_sum = 0
                exponent_activation_sum = np.sum(np.exp(activation_matrix))
                for activation_raw_index in range(len(activation_matrix)):
                    activation_matrix[activation_raw_index] = np.exp(activation_matrix[activation_raw_index]) / exponent_activation_sum
        else:
...
# shuffle data
inputs_shuffled, expected_outputs_shuffled = shuffle_data(inputs, expected_outputs)

# separate the input and expected data into training and testing categories
X_train, Y_train, X_test, Y_test = train_test_split(inputs_shuffled, expected_outputs_shuffled, 0.8)
